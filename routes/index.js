...
router.post('/journal', ensureAuthenticated, function(req, res, next) {
	...
	async.parallel({
  	...
  	categories: function(callback) {
  		...
  		if (err) { return next(err); }
  		console.log(results.entries);
  		res.render('journal', { title: 'Frustrations Journal', user:req.user, entries: results.entries, categories: results.categories, filtered: true });
	});
})
...
router.post('/profile/change-email', ensureAuthenticated, [
	check('new_username', 'Please enter a valid email address.').isLength({min:1}).isEmail().trim(),
		...
		if(!errors.isEmpty()) {
			res.render('profile', {title: 'My Profile', errors:errors.array(), username: username, user:req.user});
			return;
		} else {
...
router.post('/profile/change-password', ensureAuthenticated, [
		...
		if(!errors.isEmpty()) {
			res.render('profile', {title: 'My Profile', errors:errors.array()});
			return;
		} else if (new_password !== match_new_password) {
			res.render('profile', {title: 'My Profile', error_type: 'password_match', message: 'This should match your new password.'});
		} else {
